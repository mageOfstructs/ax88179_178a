 ...
 * Copyright (C) 2021 Pingmin Fenlly Liu <pingmin(at)fenlly.org>, Herman
 * <GermanAizek@yandex.ru> Copyright (C) 2003-2005 David Hollis
 * <dhollis@davehollis.com> Copyright (C) 2005 Phil Chang
 * <pchang23@sbcglobal.net> Copyright (c) 2002-2003 TiVo Inc.
 *
...
static char version[] = KERN_INFO "ASIX USB Ethernet Adapter:v" DRIVER_VERSION
                                  //	" " __TIME__ " " __DATE__ "\n"
...
static int __ax88179_read_cmd(struct usbnet *dev, u8 cmd, u16 value, u16 index,
  ...
  ret = fn(dev, cmd, USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE, value,
           index, data, size);

  if (unlikely(ret < 0))
    netdev_warn(dev->net, "Failed to read reg index 0x%04x: %d\n", index, ret);
#else
  ret = usb_control_msg(dev->udev, usb_rcvctrlpipe(dev->udev, 0), cmd,
...
static int __ax88179_write_cmd(struct usbnet *dev, u8 cmd, u16 value, u16 index,
  ...
  ret = fn(dev, cmd, USB_DIR_OUT | USB_TYPE_VENDOR | USB_RECIP_DEVICE, value,
           index, data, size);

  if (unlikely(ret < 0))
    netdev_warn(dev->net, "Failed to write reg index 0x%04x: %d\n", index, ret);
#else
  ret = usb_control_msg(dev->udev, usb_sndctrlpipe(dev->udev, 0), cmd,
...
static int ax88179_write_cmd_nopm(struct usbnet *dev, u8 cmd, u16 value,
  ...
  if (2 == size) {
    ...
    buf = *((u16 *)data);
    cpu_to_le16s(&buf);
    ret = __ax88179_write_cmd(dev, cmd, value, index, size, &buf, 1);
  } else {
    ret = __ax88179_write_cmd(dev, cmd, value, index, size, data, 1);
  }
...
static int ax88179_write_cmd(struct usbnet *dev, u8 cmd, u16 value, u16 index,
  ...
  if (2 == size) {
    ...
    buf = *((u16 *)data);
    cpu_to_le16s(&buf);
    ret = __ax88179_write_cmd(dev, cmd, value, index, size, &buf, 0);
  } else {
    ret = __ax88179_write_cmd(dev, cmd, value, index, size, data, 0);
  }
...
#endif
{
  ...
  if (urb->status < 0)
    printk(KERN_ERR "ax88179_async_cmd_callback() failed with %d", urb->status);
...
static void ax88179_write_cmd_async(struct usbnet *dev, u8 cmd, u16 value,
  ...
  asyncdata = (struct ax88179_async_handle *)kmalloc(
      sizeof(struct ax88179_async_handle), GFP_ATOMIC);
  if (asyncdata == NULL) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 34)
  ...
  usb_fill_control_urb(urb, dev->udev, usb_sndctrlpipe(dev->udev, 0),
                       (void *)req, buf, size, ax88179_async_cmd_callback,
                       asyncdata);
...
#endif
{
  ...
  /* Disable RX path */
  ax88179_read_cmd_nopm(dev, AX_ACCESS_MAC, AX_MEDIUM_STATUS_MODE, 2, 2, &tmp16,
                        1);
  tmp16 &= ~AX_MEDIUM_RECEIVE_EN;
  ax88179_write_cmd_nopm(dev, AX_ACCESS_MAC, AX_MEDIUM_STATUS_MODE, 2, 2,
                         &tmp16);

  /* Force bz */
  ax88179_read_cmd_nopm(dev, AX_ACCESS_MAC, AX_PHYPWR_RSTCTL, 2, 2, &tmp16, 1);
  tmp16 |= AX_PHYPWR_RSTCTL_BZ | AX_PHYPWR_RSTCTL_IPRL;
  ax88179_write_cmd_nopm(dev, AX_ACCESS_MAC, AX_PHYPWR_RSTCTL, 2, 2, &tmp16);
...
static void ax88179_EEE_setting(struct usbnet *dev) {
  ...
  if (bEEE) {
    // Enable EEE
    tmp16 = 0x07;
    ax88179_write_cmd(dev, AX_ACCESS_PHY, AX88179_PHY_ID, GMII_PHY_MACR, 2,
                      &tmp16);

    tmp16 = 0x3c;
    ax88179_write_cmd(dev, AX_ACCESS_PHY, AX88179_PHY_ID, GMII_PHY_MAADR, 2,
                      &tmp16);
...
static void ax88179_Gether_setting(struct usbnet *dev) {
  ...
  if (bGETH) {
    ...
    tmp16 = 0x03;
    ax88179_write_cmd(dev, AX_ACCESS_PHY, AX88179_PHY_ID, 31, 2, &tmp16);

    tmp16 = 0x3247;
    ax88179_write_cmd(dev, AX_ACCESS_PHY, AX88179_PHY_ID, 25, 2, &tmp16);

    tmp16 = 0x05;
    ax88179_write_cmd(dev, AX_ACCESS_PHY, AX88179_PHY_ID, 31, 2, &tmp16);

    tmp16 = 0x0680;
    ax88179_write_cmd(dev, AX_ACCESS_PHY, AX88179_PHY_ID, 1, 2, &tmp16);
...
static int ax88179_resume(struct usb_interface *intf) {
  ...
  /* change clock */
  ax88179_read_cmd_nopm(dev, AX_ACCESS_MAC, AX_CLK_SELECT, 1, 1, &tmp8, 0);
...
static void ax88179_get_wol(struct net_device *net,
...
static int ax88179_set_wol(struct net_device *net,
                           struct ethtool_wolinfo *wolinfo) {
  struct usbnet *dev = netdev_priv(net);
  struct ax88179_data *ax179_data = (struct ax88179_data *)dev->data;
  u8 opt = 0;
...
static int ax88179_get_eeprom(struct net_device *net,
  ...
  eeprom_buff =
      kmalloc_array(last_word - first_word + 1, sizeof(u16), GFP_KERNEL);
  if (!eeprom_buff)
    return -ENOMEM;
...
static int ax88179_get_settings(struct net_device *net,
                                struct ethtool_cmd *cmd) {
  struct usbnet *dev = netdev_priv(net);
  return mii_ethtool_gset(&dev->mii, cmd);
}

static int ax88179_set_settings(struct net_device *net,
                                struct ethtool_cmd *cmd) {
  struct usbnet *dev = netdev_priv(net);
  return mii_ethtool_sset(&dev->mii, cmd);
}
#else
static int ax88179_get_link_ksettings(struct net_device *netdev,
...
static int
ax88179_set_link_ksettings(struct net_device *netdev,
...
static int ax88179_netdev_stop(struct net_device *net) {
  ...
  ax88179_read_cmd(dev, AX_ACCESS_MAC, AX_MEDIUM_STATUS_MODE, 2, 2, tmp16, 1);
  *tmp16 &= ~AX_MEDIUM_RECEIVE_EN;
...

{
  ...
  if (changed & NETIF_F_IP_CSUM) {
    ax88179_read_cmd(dev, AX_ACCESS_MAC, AX_TXCOE_CTL, 1, 1, tmp8, 0);
  ...
  if (changed & NETIF_F_RXCSUM) {
    ax88179_read_cmd(dev, AX_ACCESS_MAC, AX_RXCOE_CTL, 1, 1, tmp8, 0);
    *tmp8 ^= AX_RXCOE_IP | AX_RXCOE_TCP | AX_RXCOE_UDP | AX_RXCOE_TCPV6 |
             AX_RXCOE_UDPV6;
...
static int ax88179_set_mac_addr(struct net_device *net, void *p) {
  ...
  /* Set the MAC address */
  ret = ax88179_write_cmd(dev, AX_ACCESS_MAC, AX_NODE_ID, ETH_ALEN, ETH_ALEN,
                          net->dev_addr);
  if (ret < 0)
    return ret;
...
static int ax88179_check_eeprom(struct usbnet *dev) {
  ...
  for (i = 0; i < 6; i++) {
    ...
    do {
      ax88179_read_cmd(dev, AX_ACCESS_MAC, AX_SROM_CMD, 1, 1, buf, 0);
...
static int ax88179_convert_old_led(struct usbnet *dev, u8 efuse,
                                   void *ledvalue) {
  u8 ledmode = 0;
  u16 *tmp16;
  u16 led = 0;
  ...
  if (efuse) {
    if (ax88179_read_cmd(dev, AX_ACCESS_EFUSE, 0x18, 1, 2, tmp16, 1) < 0) {
      kfree(tmp16);
      return -EINVAL;
    }
    ledmode = (u8)(*tmp16 & 0xFF);
  } else { /* loaded the old EEprom LED Mode */
    if (ax88179_read_cmd(dev, AX_ACCESS_EEPROM, 0x3C, 1, 2, tmp16, 1) < 0) {
      kfree(tmp16);
      return -EINVAL;
    }
    ledmode = (u8)(*tmp16 >> 8);
  ...
  case 0xFD:
    led =
        LED0_ACTIVE | LED1_LINK_1000 | LED2_LINK_100 | LED2_LINK_10 | LED_VALID;
    break;
  case 0xFC:
...
static int ax88179_led_setting(struct usbnet *dev) {
  ...
  if (ax88179_check_eeprom(dev) == AX_EEP_EFUSE_CORRECT) {
    ...
    *value = EEP_RD;
    ...
    jtimeout = jiffies + delay;
    do {
      ax88179_read_cmd(dev, AX_ACCESS_MAC, AX_SROM_CMD, 1, 1, value, 0);
    ...
    ax88179_read_cmd(dev, AX_ACCESS_MAC, AX_SROM_DATA_HIGH, 1, 1, value, 0);
    ledvalue = (*value << 8);
    ax88179_read_cmd(dev, AX_ACCESS_MAC, AX_SROM_DATA_LOW, 1, 1, value, 0);
    ledvalue |= *value;
  ...
  *tmp16 = GMII_PHY_PAGE_SELECT_EXT;
  ax88179_write_cmd(dev, AX_ACCESS_PHY, AX88179_PHY_ID, GMII_PHY_PAGE_SELECT, 2,
                    tmp16);

  *tmp16 = 0x2c;
  ax88179_write_cmd(dev, AX_ACCESS_PHY, AX88179_PHY_ID, GMII_PHYPAGE, 2, tmp16);

  ax88179_read_cmd(dev, AX_ACCESS_PHY, AX88179_PHY_ID, GMII_LED_ACTIVE, 2,
                   ledact, 1);

  ax88179_read_cmd(dev, AX_ACCESS_PHY, AX88179_PHY_ID, GMII_LED_LINK, 2,
                   ledlink, 1);
  ...
  *ledlink &= GMII_LED_LINK_MASK;
  ...
  ax88179_write_cmd(dev, AX_ACCESS_PHY, AX88179_PHY_ID, GMII_LED_ACTIVE, 2,
                    ledact);

  ax88179_write_cmd(dev, AX_ACCESS_PHY, AX88179_PHY_ID, GMII_LED_LINK, 2,
                    ledlink);
...
static int access_eeprom_mac(struct usbnet *dev, u8 *buf, u8 offset,
                             int wflag) {
  int ret = 0, i;
  u16 *tmp = (u16 *)buf;
  u16 *tmp16;
  ...
  for (i = 0; i < (ETH_ALEN >> 1); i++) {
    if (wflag) {
      *tmp16 = cpu_to_le16(*(tmp + i));
      ret = ax88179_write_cmd(dev, AX_ACCESS_EEPROM, offset + i, 1, 2, tmp16);
      if (ret < 0)
        break;
      ...
      ret =
          ax88179_read_cmd(dev, AX_ACCESS_EEPROM, offset + i, 1, 2, tmp + i, 0);
      if (ret < 0)
        break;
    }
  }
...
static int ax88179_get_mac(struct usbnet *dev, u8 *buf) {
  ...
  if (ax88179_check_ether_addr(dev)) {
    ...
    ret = ax88179_read_cmd(dev, AX_ACCESS_MAC, AX_NODE_ID, ETH_ALEN, ETH_ALEN,
                           buf, 0);
    if (ret < 0) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 34)
...
static int ax88179_bind(struct usbnet *dev, struct usb_interface *intf) {
  ...
  devdbg(dev, "MAC [%02x-%02x-%02x-%02x-%02x-%02x]\n", dev->net->dev_addr[0],
         dev->net->dev_addr[1], dev->net->dev_addr[2], dev->net->dev_addr[3],
         dev->net->dev_addr[4], dev->net->dev_addr[5]);
#endif
  ...
  tmp = AX_RXCOE_IP | AX_RXCOE_TCP | AX_RXCOE_UDP | AX_RXCOE_TCPV6 |
        AX_RXCOE_UDPV6;
  ax88179_write_cmd(dev, AX_ACCESS_MAC, AX_RXCOE_CTL, 1, 1, &tmp);

  tmp = AX_TXCOE_IP | AX_TXCOE_TCP | AX_TXCOE_UDP | AX_TXCOE_TCPV6 |
        AX_TXCOE_UDPV6;
...
static void ax88179_rx_checksum(struct sk_buff *skb, u32 *pkt_hdr) {
  ...
  if ((*pkt_hdr & AX_RXHDR_L3CSUM_ERR) || (*pkt_hdr & AX_RXHDR_L4CSUM_ERR))
    return;
...
static int ax88179_rx_fixup(struct usbnet *dev, struct sk_buff *skb) {
  ...
  while (pkt_cnt--) {
    ...
    if ((*pkt_hdr & AX_RXHDR_CRC_ERR) || (*pkt_hdr & AX_RXHDR_DROP_ERR)) {
...
static struct sk_buff *ax88179_tx_fixup(struct usbnet *dev, struct sk_buff *skb,
                                        gfp_t flags) {
...
static int ax88179_link_reset(struct usbnet *dev) {
  ...
  *mode = AX_MEDIUM_TXFLOW_CTRLEN | AX_MEDIUM_RXFLOW_CTRLEN;
  ...
  ax88179_read_cmd(dev, AX_ACCESS_MAC, PHYSICAL_LINK_STATUS, 1, 1, link_sts, 0);

  jtimeout = jiffies + delay;
  while (time_before(jiffies, jtimeout)) {

    ax88179_read_cmd(dev, AX_ACCESS_PHY, AX88179_PHY_ID, GMII_PHY_PHYSR, 2,
                     tmp16, 1);
...
static int __init asix_init(void) { return usb_register(&asix_driver); }
module_init(asix_init);

static void __exit asix_exit(void) { usb_deregister(&asix_driver); }
